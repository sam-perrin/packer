...
variable "iso_file" {
  type    = string
  description = "The file name of the ISO image. (e.g. 'iso-redhat-server-7.iso')"
  default = ""
}

variable "iso_checksum" {
  type    = string
  description = "The SHA-512 checkcum of the ISO image. (e.g. Result of 'shasum -a 512 iso-redhat-server-7.iso')"
  default = ""
}
...
source "vsphere-iso" "linux-redhat-server" {
...
build {
  sources = ["source.vsphere-iso.linux-redhat-server"]
  /*
